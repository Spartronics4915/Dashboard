...
export class RobotStateMgr
{
    ...
    addRandomPose()
    {
        // console.log("addRandomPose"); (only called when in demo mode)
        if(this.activeList != undefined)
        {
            ...
            else
            {
                ...
                if(x > field.xrange[1])
                    rads = -Math.PI; // point left
                else
                if(y < field.yrange[0])
                    rads = Math.PI / 2; // point down
                else
                if(y > field.yrange[1])
                    rads = -Math.PI / 2; // point up
                else
